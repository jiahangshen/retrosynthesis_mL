...
def get_tpl(task):
    idx, react, prod, id, cls,domain_id,rxn = task
    ...
    return idx, react, prod, reaction_smarts, cano_react, retro_okay_list, id, cls,domain_id,rxn
...
if __name__ == "__main__":
    ...
    for data_set in ['valid', 'train', 'test']:
        ...
        if not os.path.isfile(data_file):
            ...
            rxns = []
            domain_mapping = {}  # 需要先定义领域映射
            for idx, r in enumerate(reactant_list):
                domain_id = domain_mapping.get(classes[idx], classes[idx])  # 保持原始类别或映射后的领域ID
                rxns.append((idx, r, product_list[idx], ids[idx], classes[idx],domain_id,reaction_list[idx]))
            ...
            for result in tqdm(pool.imap_unordered(get_tpl, rxns), total=len(rxns)):
                idx, react, prod, reaction_smarts, cano_reacts, retro_okay, id, cls ,domain_id,reaction= result
                ...
                train_templates[idx] = {
                    ...
                    'domain_id': domain_id if domain_id is not None else 0,
                    'reaction':reaction,
                }
    ...
    if False and os.path.isfile(templates_cano_train):
        ...
        # 新增域标签收集结构
        reaction_domains = {}  # 模板 -> 域标签映射
        domain_labels = []     # 与reaction_smarts保持顺序一致
        reactions=[]
        for idx, val in tqdm(templates_data_train.items()):
            ...
            if len(items) == 2 and items[0] and items[1]:
                ...
                domain = val.get('domain_id', 0)  # 假设域标签字段为domain_id，默认0
                reaction_domains[reaction] = domain
        ...
        for idx, val in tqdm(templates_data_train.items()):
            if 'cano_reaction_smarts' in val:
                ...
                domain_labels.append(reaction_domains[reaction])  # 同步添加域标签
        tmpl_lens = []
        templates_train = set()
        ...
        data = {
            'reactions':reactions,
            'domain_labels': domain_labels,  # 新增域标签列表
