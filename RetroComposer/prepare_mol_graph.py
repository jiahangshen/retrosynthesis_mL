...
from chemutils import cano_smarts, cano_smarts_, cano_smiles, cano_smiles_
from extract_templates import Reactor
from process_templates import compose_tmpl
...
class MoleculeDataset(Dataset):
    def __init__(self, root, split='train', load_mol=False, min_k_prod=0, min_k_react=0 , DomainML=True):
        """
        ...
        if load_mol:
            ...
            self.reactions = molecules['reactions']
            self.domain_labels = molecules.get('domain_labels', [0]*len(self.reactions))  # 默认0
            unique_reactions = set(self.reactions)
            self.reaction_to_domain = dict(zip(
                self.reactions, 
                self.domain_labels
            ))
            self.unique_domains = sorted(list(set(self.domain_labels)))
            self.domain_to_idx = {d: i for i, d in enumerate(self.unique_domains)}
            ...
            if os.path.exists(self.seq_to_templates_file):
                ...
                print('can not find seq_to_templates file:', self.seq_to_templates_file)
                if self.split == 'train':
                    skipped = 0
                    ...
                    print('unique seq_to_templates size:', len(self.seq_to_templates))
                    print('skipped train rxns:', skipped)
    ...
    def process_data(self):
        ...
        templates_file = os.path.join(self.root, 'templates_{}.json'.format(self.split))
        print('process datafile:', templates_file)
        ...
        for idx, val in tqdm(templates.items()):
            ...
            if self.DomainML:
                ...
                domain_id = self.reaction_to_domain.get(reaction, 0)
                if domain_id is None:  
                    domain_id = 0
                gnn_data.domain_labels = torch.tensor([domain_id], dtype=torch.long)
            ...
            reaction_center_atom_indexes = torch.zeros((len(val['reaction_center_atom_indexes']), gnn_data.atom_len), dtype=torch.bool)
            ...
            processed_data_file = os.path.join(self.processed_dir, '{}_{}.data'.format(self.split, idx))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--retro', action='store_true',default=True, help='prepare retro data or ranking data')
